...
const char * const BITCOIN_CONF_FILENAME = "bitcoinrocks.conf";
...
fs::path GetDefaultDataDir()
{
    ...
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinRocks";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "BitcoinRocks";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/BitcoinRocks";
#else
    // Unix-like
    return pathRet / ".bitcoinrocks";
#endif
#endif
}
